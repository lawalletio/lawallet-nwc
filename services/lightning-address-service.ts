...
export class LightningAddressService {
  ...
  static get(username: string): LightningAddress | null {
    const address = mockLightningAddressData.find((addr) => addr.username === username)
    return address || null
  }
  ...
  static getByPubkey(pubkey: string): LightningAddress | null {
    const address = mockLightningAddressData.find((addr) => addr.pubkey === pubkey)
    return address || null
  }
  ...
  static searchByUsername(query: string): LightningAddress[] {
    ...
    return mockLightningAddressData.filter((addr) => addr.username.toLowerCase().includes(lowercaseQuery))
  }
  ...
  static getByRelay(relayUrl: string): LightningAddress[] {
    return mockLightningAddressData.filter((addr) => addr.nwc && addr.nwc.includes(encodeURIComponent(relayUrl)))
  }
