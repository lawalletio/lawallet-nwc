...
const activeWallet = {
  ...
  type: 'NWC' as const,
  config: {
    connectionString: 'nostr+walletconnect://bound',
    mode: 'SEND_RECEIVE'
  },
...
describe('resolveWalletRoute', () => {
  ...
  it('returns idle for IDLE', () => {
    expect(resolveWalletRoute({ ...base, mode: 'IDLE' })).toEqual({
      kind: 'idle'
    })
  })
  ...
  it('returns unconfigured for ALIAS without a redirect', () => {
    expect(resolveWalletRoute({ ...base, mode: 'ALIAS' })).toEqual({
      kind: 'unconfigured'
    })
  })
...
describe('resolveCardWallet', () => {
  ...
  it('falls back to the default RemoteWallet when the card has no binding', () => {
    expect(
      resolveCardWallet({ ...base, defaultRemoteWallet: activeWallet })
    ).toEqual({
      kind: 'wallet',
      walletId: activeWallet.id,
      type: 'NWC',
