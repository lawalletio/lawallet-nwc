...
function seedMergeUsers(overrides?: {
  ...
  vi.mocked(prismaMock.user.findUnique).mockImplementation(
    (args: any) =>
      Promise.resolve(
        args.where.id === A ? survivor : args.where.id === B ? absorbed : null
      ) as any
  )
...
function seedMergeDefaults() {
  ...
  vi.mocked(prismaMock.passkeyCredential.updateMany).mockResolvedValue(
    count as any
  )
...
describe('mergeAccounts', () => {
  ...
  it('unions both relay lists onto the survivor', async () => {
    seedMergeUsers({
      survivor: {
        relays: JSON.stringify(['wss://a.example', 'wss://shared.example'])
      },
      absorbed: {
        relays: JSON.stringify(['wss://b.example', 'wss://shared.example'])
      }
    })
    seedMergeDefaults()
...
describe('identity mutations', () => {
  ...
  it('linkPubkeyToAccount maps P2002 to 409', async () => {
    vi.mocked(prismaMock.nostrIdentity.create).mockRejectedValue({
      code: 'P2002'
    })
    await expect(linkPubkeyToAccount(A, PK_B)).rejects.toBeInstanceOf(
      ConflictError
    )
  })
  ...
  it('setPrimaryIdentity 404s for a foreign identity', async () => {
    ...
    await expect(setPrimaryIdentity(A, PK_B)).rejects.toBeInstanceOf(
      NotFoundError
    )
  })
