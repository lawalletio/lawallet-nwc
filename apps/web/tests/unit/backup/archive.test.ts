...
describe('backup archive (parseBackupFile)', () => {
  it('returns the manifest + parsed rows for a valid archive', async () => {
    ;(
      prismaMock.settings.findMany as ReturnType<typeof vi.fn>
    ).mockResolvedValue([
      {
        name: 'domain',
        value: 'example.com',
    ...
    expect((parsed.tables.settings as Record<string, unknown>[])[0].name).toBe(
      'domain'
    )
  })
  ...
  it('throws when a table checksum is tampered', async () => {
    const bytes = buildZip(
      ...
      files => {
        ...
        files['tables/settings.ndjson'] = utf8Encode(
          '{"name":"a","value":"TAMPERED"}'
        )
      }
    )
  ...
  it('throws a password-required error for an encrypted archive with no password', async () => {
    ...
    await expect(
      parseBackupFile(toFile(encrypted, 'backup.zip.enc'))
  ...
  it('throws when a manifest-listed table file is absent', async () => {
    ...
    const bytes = zipSync({
      'manifest.json': utf8Encode(JSON.stringify(manifest))
    })
    await expect(parseBackupFile(toFile(bytes))).rejects.toThrow(
      /missing data for/
    )
  ...
  it('throws when the bytes are not a valid zip at all', async () => {
    await expect(
      parseBackupFile(toFile(utf8Encode('not a zip'), 'x.zip'))
    ).rejects.toThrow(/not a valid backup archive/)
  })
})
