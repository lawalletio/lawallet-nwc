...
export function TableSummaryGrid({
  tables
}: {
  tables: BackupAnalyzeResponse['tables']
}) {
  const entries = Object.entries(tables) as [
    BackupTableName,
    BackupAnalyzeResponse['tables'][BackupTableName]
  ][]
  const nonEmpty = entries.filter(
    ([, analysis]) => (analysis?.counts.total ?? 0) > 0
  )

  if (nonEmpty.length === 0) {
    return (
      <p className="text-sm text-muted-foreground">
        This backup contains no records to restore.
      </p>
    )
  }
          ...
          >
            ...
            <div className="mt-2 flex flex-wrap gap-1.5">
              ...
              {counts.conflicting > 0 && (
                ...
                >
                  {counts.conflicting} conflict
                  {counts.conflicting === 1 ? '' : 's'}
                </Badge>
              )}
              {counts.identical > 0 && (
