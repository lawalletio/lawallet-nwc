...
import { type TableDescriptor, fieldsKey, pkKey } from '@/lib/backup/tables'
...
export async function loadTableContext(
  ...
  for (const pu of desc.partialUniques) {
    ...
    if (scoped.length > 0) {
      const combos = scoped.map(r =>
        Object.fromEntries(pu.scope.map(f => [f, r[f]]))
      )
...
export function classifyRow(
  ...
  if (existing) {
    ...
    return {
      ...
      conflict: makeConflict(desc, rowKey, {
        ...
        existingOwnerId:
          typeof existing.userId === 'string' ? existing.userId : undefined,
  ...
  for (const su of desc.secondaryUniques) {
    ...
    const hit = ctx.secondaryExisting
      .get(su.label)
      ?.get(fieldsKey(su.fields, row))
    if (hit) {
      const renameable =
        Boolean(desc.renameField) && su.fields.includes(desc.renameField!)
      return {
        status: 'conflicting',
        conflict: makeConflict(desc, rowKey, {
          ...
          existingOwnerId:
            typeof hit.userId === 'string' ? hit.userId : undefined,
  ...
  for (const pu of desc.partialUniques) {
    const applies = pu.flag
      ? row[pu.flag] === true
      : row[pu.where!.field] === pu.where!.equals
    if (!applies) continue
