...
/**
 * `GET {callback}` — LUD-16 pay.
 *
 * Extracted verbatim from the route handler so the callback can dispatch more
 * than one action. The payment path itself is unchanged; that is what
 * `tests/integration/api/lud16.test.ts` guards.
 */
export default async function pay(
  ...
  const zap = nostr ? validateZapRequest({ raw: nostr, amountMsats }) : null
