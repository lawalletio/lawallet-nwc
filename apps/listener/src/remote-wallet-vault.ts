...
export function decryptRemoteWalletNwcUri(
  ...
  const buf = Buffer.from(stored.slice(ENVELOPE_PREFIX.length), 'base64url')
  ...
  for (const secret of [env.NWC_VAULT_SECRET, ...previous]) {
    try {
      ...
      decipher.setAAD(Buffer.from(`${walletId}:connection-string`, 'utf8'))
