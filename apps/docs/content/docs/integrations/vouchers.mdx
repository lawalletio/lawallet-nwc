...
Every field except `npub`, `nonce`, `name`, `merchantPubkey`, and `claimUrl` is optional. `npub`, `merchantPubkey`, and `servicePubkey` accept either an `npub1…` or a 64-character hex pubkey.
