...
Validation is **signature + `exp` only** — there is **no session record and no revocation**.
Lifetimes are therefore deliberately short and bounded server-side to **1 minute – 30 days**. To
cut a device off early, let the token expire (or rotate the `JWT_SECRET`, which invalidates every
token) and mint a fresh one.
