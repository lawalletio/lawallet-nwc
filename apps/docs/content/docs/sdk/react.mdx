---
title: 'React Hooks'
description: 'LaWalletProvider and every hook in @lawallet-nwc/react, with copy-pasteable examples.'
---

`@lawallet-nwc/react` wraps the SDK in a provider + hooks. One prop — your
...
`@lawallet-nwc/react` and you never need a second package.
...
```tsx
import { useAuth, hasBrowserExtension } from '@lawallet-nwc/react'
...
custom) — see [Authentication](/docs/sdk/authentication). There is no token
and no refresh: logging in just hands the client a signer.

## useUser — signup is a side effect

```tsx
import { useUser } from '@lawallet-nwc/react'
...
```tsx
import { useAddresses, useAddress, LaWalletError } from '@lawallet-nwc/react'
...
[`useClaimAddress`](/docs/sdk/registration-flow).

## useUsernameAvailability — check as they type

```tsx
import { useUsernameAvailability } from '@lawallet-nwc/react'
...
```tsx
import { useAddressInvoices } from '@lawallet-nwc/react'
...
```tsx
import { useRemoteWallets } from '@lawallet-nwc/react'
...
See [NWC & routing](/docs/sdk/nwc) for binding a wallet to an address.
...
state machine. Full guide: [Registration flow](/docs/sdk/registration-flow).
...
```tsx
import { useLaWallet, useResource } from '@lawallet-nwc/react'
