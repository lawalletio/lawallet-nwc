...
<a
  href="https://app.netlify.com/start/deploy?repository=https://github.com/lawalletio/lawallet-nwc"
  target="_blank"
...
<Callout type="warn" title="Copy a connection string that supports migrations">
  ...
  **transaction-mode poolers reject** — deploys fail with `prepared statement
  "s0" already exists`. That rules out Neon's `-pooler` hostname and Supabase's
...
3. Turn **Connection pooling** _off_ — the hostname must **not** contain
   `-pooler`.
...
Use `lawallet-cloud-env/web.env` for Netlify and
`lawallet-cloud-env/listener.env` on the always-on listener host. Replace the
database and URL placeholders first. The
[Environment Variables guide](/docs/deploy/environment) explains each value.
...
3. Fill in the three environment variables Netlify asks for:
   ...
   | `NWC_VAULT_SECRET` | A different random string of 32+ characters; keep it stable and backed up |
...
| Variable                                    | Purpose                                                                                                                                       |
| ------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `NWC_VAULT_SECRET_PREVIOUS`                 | Comma-separated old NWC vault keys accepted temporarily during a controlled key rotation. Set the same value on the listener.                 |
