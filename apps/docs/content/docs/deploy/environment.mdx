...
Back both up and keep them stable. Recovery from a changed secret is per
credential, not automatic:
...
Do not change or remove proxy credentials while non-completed proxy payments
exist.
...
replaces it and updates the advertised pubkey, so zaps recover with no
operator action.

A replacement changes the instance's `_` identity but destroys nothing: the
displaced ciphertext is kept verbatim in `receiptNsecRetiredCiphertext`
alongside `receiptPubkeyRetired` and `receiptSignerReplacedAt`. If you still
have the secret that sealed it, decrypt that column and re-enter the key in
Admin → Settings → NWC Services to restore the original identity. Watch for
