...
rows, converts leftover `LWPX01` proxy blobs when they still decrypt, verifies
existing envelopes against the current secret, and refuses to start if NWC
rows or proxy NWC exist but the key is missing or the ciphertext will not
open. An unreadable receipt nsec is left in place and NIP-57 degrades. The
listener must use the same key so it can open the connections.
