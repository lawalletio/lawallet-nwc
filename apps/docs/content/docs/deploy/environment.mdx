...
Sentry integration is **entirely optional and off by default**. Self-hosters
who do not use Sentry leave all three variables unset — nothing is collected
and no Sentry code is activated.
...
`NWC_VAULT_SECRET_PREVIOUS` does the same for the NWC vault: set the old
secret (or a comma-separated list) alongside the new `NWC_VAULT_SECRET`, and
startup re-seals every credential it can open under the active secret. Once
the boot log shows no unreadable credentials, the previous value can be
dropped on the following deploy. Set it on the listener too — it may boot
before web finishes re-sealing.

Rotating without it is what makes existing RemoteWallet and proxy credentials
unreadable, so keep the old value until the re-seal has run. Do not rotate or
remove proxy credentials while non-completed proxy payments exist.
...
zaps off for _every_ NWC wallet. Startup guarantees a usable one: it generates
a signer when the config row has none, derives a missing pubkey from a
readable key, and — only when no configured secret can open the stored key —
replaces it and updates the advertised pubkey.

Prefer recovery over replacement: set `NWC_VAULT_SECRET_PREVIOUS` and the
original key is re-sealed under the active secret, keeping the instance's `_`
identity. Replacement is the last resort, and even then nothing is destroyed —
the displaced ciphertext is kept verbatim in `receiptNsecRetiredCiphertext`
alongside `receiptPubkeyRetired` and `receiptSignerReplacedAt`, so restoring
the right secret can still recover the original identity. Watch for
`proxy_receipt_signer.replaced_unreadable` in the logs.
