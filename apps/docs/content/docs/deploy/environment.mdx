...
turns zaps off for _every_ NWC wallet. Since it is a key the platform
generates for itself and nothing can recover the old one, startup replaces it
with a fresh key and updates the advertised pubkey — but only after another
NWC credential has proved the active secret is the right one, so a
temporarily misconfigured `NWC_VAULT_SECRET` can never destroy a recoverable
key. Watch for `proxy.receipt_signer.replaced_unreadable` in the logs.
