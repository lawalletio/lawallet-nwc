...
| `DEAD_WALLET_DETECTION_ENABLED` | —        | Wallet auto-archival, both paths (default `true`; set `false` to disable). Probe path: a disposable LNCurl wallet that stops answering for `DEAD_THRESHOLD_HOURS` with relays up. Idle path: ANY wallet with no sign of life for `WALLET_ARCHIVE_IDLE_HOURS`, including one that never completed warm-up. |
| `DEAD_THRESHOLD_HOURS`          | —        | Silence (relays up) before an LNCurl wallet is declared dead (default 4).                                                                                                                                             |
| `WALLET_ARCHIVE_IDLE_HOURS`     | —        | Idleness before any wallet is reported for archival (default 48 — the product rule; web enforces 48h independently).                                                                                                  |
| `WALLET_ARCHIVE_RETRY_MS`       | —        | How long a reported wallet stays parked: its reconnect backoff and report retry (default 21600000 = 6h).                                                                                                              |
