...
2. From the **repo root**, provision and deploy in one step:
   ...
   It refuses a pooled connection string, generates
   `LISTENER_AUTH_SECRET` and `NWC_VAULT_SECRET` on first run (leaving any
   secret already set on the app untouched), and prints the generated pair
   once — you need both on the web side.
