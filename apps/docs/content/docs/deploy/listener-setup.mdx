...
1. From the **repo root**, create the app: `fly apps create lawallet-listener`.
2. Set the secrets (all four are required — the service refuses to boot
   without them):

   ```bash
   fly secrets set --app lawallet-listener \
     DATABASE_URL="postgresql://…" \
     LISTENER_AUTH_SECRET="<same secret as the web app>" \
     NWC_VAULT_SECRET="<same NWC vault secret as the web app>" \
     WEB_ORIGIN="https://wallet.example.com"
   ```

3. Deploy, still from the repo root so the build context is the whole
   workspace: `fly deploy --config apps/listener/fly.toml`.
4. Paste `https://lawallet-listener.fly.dev` and the shared secret into
   **Settings → NWC Services** and hit **Test connection**.
