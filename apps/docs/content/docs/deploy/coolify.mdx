...
Open `lawallet.coolify.env`, copy every entry into Coolify's **Environment
...
See [Environment Variables](/docs/deploy/environment) for a description of
every value and rotation guidance.
...
across upgrades and backups. During a controlled rotation, set
`NWC_VAULT_SECRET_PREVIOUS` to the previous value on both services until stored
credentials have been rewritten. The NIP-57 receipt signer is an `nsec`
configured in **Admin → Settings → NWC Services**, not a Coolify environment
variable.
