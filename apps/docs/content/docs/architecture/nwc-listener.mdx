...
- **RemoteWallet forwarding journal** — personal receive-forwarding legs use
  the same durable request journal and deterministic request IDs. The webhook
  records the source receipt before acknowledgement; the immediate wake-up and
  ten-minute scheduler drive the owner-scoped forwarding reconciler.
...
- **Idle auto-archive** — a wallet with no sign of life for
  `WALLET_ARCHIVE_IDLE_HOURS` (48) is reported for archival even when it can't
  be probed at all, which is the only signal a wallet whose NWC warm-up never
  succeeded can ever give. The clock lives in Postgres, so a restart neither
  resets it nor re-reports the wallet, and a reported wallet is parked instead
  of reconnecting every 60s forever. Listener downtime is discounted from the
  measurement, so an outage longer than the window never mass-archives the pool.
