...
export const StatusInProgress = () => <span className="inline-flex items-center gap-1.5 font-semibold text-amber-600 dark:text-amber-400"><Loader2 className="size-4 animate-spin" /> In Progress</span>
...
| 5 | Expansion | <StatusInProgress /> | Card System completion (Android BoltCard login, simple-card-manager rebrand), full NIP-05, relay picker, user data cache, NWC Listener Lite (transport-only), LUD-22 plumbing, `@lawallet-nwc/react`, Resend, PWA, Follower Capture Endpoint |
| 6 | Expansion | <StatusPlanned /> | NWC Proxy Lite settlement layer, full LUD-16/21/22 + NIP-57 closeout, Nostr scheduler, full wallet settings, deploy targets (Vercel/Netlify/Umbrel/Start9/Docker), threat model, SDK + hooks finalization |
| 7 | Monetization | <StatusPlanned /> | **Subscription Manager** (paid tiers, monthly/one-time), perks (vanity LN address, email-to-Nostr, sat allowance), **Nostr Chat DMs** (NIP-17/44), operator → user broadcast |
