...
<Callout type="info" title="The active window">
  ...
  LUD-22 / NIP-57 closeout, `@lawallet-nwc/react` extraction, the WordPress
  plugin, the Resend adapter, the Nostr scheduler, and Vercel / Netlify deploy
  configs — then starts its own scope below. The Subscription Manager and email
  bridge build directly on the carried-over NWC Proxy Lite and Resend adapter.
</Callout>
