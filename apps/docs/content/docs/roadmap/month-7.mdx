...
<Callout type="info" title="The active window">
  Month 7 first clears the work carried over from [Month 6](./month-6) — MASTER
  card account-share (FOREVER QRs), NWC Proxy Lite + full LUD-16 / LUD-21 /
  LUD-22 / NIP-57 closeout, the WordPress plugin, the Resend adapter, the Nostr
  scheduler, and Vercel / Netlify deploy configs — then starts its own scope
  below. The **SDK + `@lawallet-nwc/react`** carry-over is already
  [delivered](/docs/sdk). The Subscription Manager and email bridge build
  directly on the carried-over NWC Proxy Lite and Resend adapter.
</Callout>
