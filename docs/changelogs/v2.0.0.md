...
It also carries a fix for a defect that made card tap payments fail _every
time_ — see [Cards](#cards).
...
**Who is actually affected:** passkey support (#101) was merged _after_ the
...
- **Multi-pubkey accounts** (#102) — a new `NostrIdentity` model plus a
  ...
  session still presents the account's _primary_ identity. Ships an
...
- **Relay bridge** with a live NWC pool, webhook fan-out, and an admin
  dashboard (#71); settings-driven configuration under a new _NWC Services_
  tab (#72).
...
- **Fixed: card tap payments failed 100% of the time** (#107).
  ...
  as an absolute Unix timestamp when it is the BOLT-11 `x` tag's _duration in
  seconds_. Every invoice therefore resolved to `expiry * 1000` ms —
  ...
  was unreachable and no invoice could pass. Validation runs _before_ the SUN
