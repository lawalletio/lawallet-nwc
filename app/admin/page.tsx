...
export default function AdminPage() {
  ...
  const stats = [
    { title: "Total Cards", value: systemStats.cardCount, description: "Active payment cards", icon: CreditCard },
    { title: "Designs", value: systemStats.cardDesignCount, description: "Available card designs", icon: Palette },
    {
      title: "Lightning Addresses",
      value: systemStats.lightningAddressCount,
  ...
  const recentCards = CardService.list().slice(0, 5)
  const recentAddresses = LightningAddressService.list().slice(0, 3)
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Welcome back! Here's an overview of your BoltCard system.</p>
        </div>
        <Button asChild>
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat) => (
          <Card key={stat.title} className="bg-card border transition-all hover:shadow-lg hover:-translate-y-1">
      ...
      <div className="grid gap-6 grid-cols-1 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>Latest cards created in your system</CardDescription>
              </div>
          ...
          <CardContent>
            {recentCards.length === 0 ? (
              <p className="text-sm text-muted-foreground">No cards created yet.</p>
            ) : (
                  ...
                  <div key={card.id} className="flex items-center justify-between p-3 bg-muted/50 rounded-md">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <p className="text-xs text-muted-foreground truncate">{card.design.description}</p>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-foreground">Lightning Addresses</CardTitle>
          ...
          <CardContent>
            {recentAddresses.length === 0 ? (
              <p className="text-sm text-muted-foreground">No addresses configured yet.</p>
            ) : (
              <div className="space-y-3">
                {recentAddresses.map((address) => (
                  <div key={address.username} className="flex items-center justify-between p-3 bg-muted/50 rounded-md">
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium text-foreground">{address.username}@yourdomain.com</p>
                      <div className="flex items-center gap-2 mt-1">
