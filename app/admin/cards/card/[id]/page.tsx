...
import { ArrowLeft, QrCode, Trash2, Download, Copy, Check, User, Nfc } from "lucide-react"
...
export default function CardPage({ params }: { params: { id: string } }) {
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-muted-foreground">The requested card could not be found.</p>
          </CardContent>
        </Card>
      </div>
    )
  ...
  const status = card.ntag424 ? (card.lastUsedAt ? "active" : "paired") : "unpaired"
      ...
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-foreground">{card.title}</h1>
              <p className="text-muted-foreground">Card details and management</p>
            </div>
          </div>
              ...
              <CardContent className="p-4">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-foreground">{card.createdAt.toLocaleDateString()}</span>
                  </div>
                  {card.lastUsedAt && (
                    <div className="flex items-center justify-between">
                      <span className="text-muted-foreground">Last Used</span>
                      <span className="text-foreground">{card.lastUsedAt.toLocaleDateString()}</span>
                    </div>
                  )}
                </div>
              </CardContent>
            ...
            <Card>
              ...
              <CardContent className="space-y-4 text-sm">
                <div className="grid gap-4 grid-cols-1 sm:grid-cols-2">
                  <div>
                    <label className="font-medium text-muted-foreground">Card ID</label>
                    <p className="font-mono text-foreground">{card.id}</p>
                  </div>
                  <div>
                    <label className="font-medium text-muted-foreground">Card Title</label>
                    <p className="text-foreground">{card.title}</p>
                  </div>
                </div>
                {card.pubkey && (
                  <div>
                    <label className="font-medium text-muted-foreground">Public Key</label>
                    <p className="font-mono text-foreground break-all">{card.pubkey}</p>
                  </div>
                )}
                {card.otc && (
                  <div>
                    <label className="font-medium text-muted-foreground">OTC</label>
            ...
            {card.ntag424 && (
              <Card>
                ...
                <CardContent className="space-y-4 text-sm font-mono">
                  <div className="grid gap-4 grid-cols-1 sm:grid-cols-2">
                    <div>
                      <label className="font-medium text-muted-foreground">Chip ID (CID)</label>
                      <p className="text-foreground">{card.ntag424.cid}</p>
                    </div>
                    <div>
                      <label className="font-medium text-muted-foreground">Counter</label>
                      <p className="text-foreground">{card.ntag424.ctr}</p>
                    </div>
                  </div>
                  {["k0", "k1", "k2", "k3", "k4"].map((key) => (
                    <div key={key}>
                      <label className="font-medium text-muted-foreground">Key {key.toUpperCase()}</label>
                      <div className="flex items-center gap-2">
                        <p className="text-foreground break-all">{card.ntag424![key as keyof typeof card.ntag424]}</p>
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-6 w-6"
                          onClick={() => handleCopy(card.ntag424![key as keyof typeof card.ntag424] as string, key)}
                        >
                          {copied === key ? (
      ...
      <Dialog open={showQRDialog} onOpenChange={setShowQRDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>QR code for card: {card.title}</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div className="text-center space-y-1">
              <p className="text-sm font-medium text-foreground">Card ID: {card.id}</p>
              <p className="text-xs text-muted-foreground break-all">{qrUrl}</p>
            </div>
            <div className="flex gap-2">
              <Button variant="outline" className="flex-1 bg-transparent" onClick={() => handleCopy(qrUrl, "qr")}>
                {copied === "qr" ? (
                  <>
